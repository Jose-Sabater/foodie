...
def meal_meals(
        meal: schemas.MealCreate, db: Session = Depends(get_db),
        current_user: int = Depends(oath2.get_current_user)
        ) -> dict:
    food_calories = db.query(models.Calories.cals_pergram).filter(
        models.Calories.fooditem == meal.meal).first()
...
def get_daterange(
    ...
    meal_query = db.query(models.Meal).filter(
...
def get_meals_ofuser(
    ...
    meals = db.query(models.Meal).filter(
        models.Meal.user_id == current_user.id).order_by(
            models.Meal.date.desc()).all()
    if not meals:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def get__last_meals(
    ...
    meals = db.query(models.Meal).filter(
        models.Meal.user_id == current_user.id).order_by(
            models.Meal.date.desc()).limit(3).all()
    if not meals:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="no meals for selected user")
...
def update_meal(
        id: int, updated_meal: schemas.MealCreate,
        db: Session = Depends(get_db)
        ) -> dict:
